...
class grid_world:
    ...
    def r_function(self,cur_state):
        cliff1 = [[0,i] for i in range(1,np.shape(self.state)[1]-1)]
        cliff2 = [[self.n_rows-1,i] for i in range(1,np.shape(self.state)[1]-1)]
        reward = -1        
        # if cur_state == self.t_state:
        #     reward = 50.

        for i in range(len(cliff1)):
            if (cur_state == cliff1[i] or cur_state == cliff2[i]):
                reward = -100
                break
...
if __name__ == '__main__':
    ...
    f.write(str(alpha))
    f.write("\t world size = ")
    f.write(str(world.n_rows))
    f.write(",")
    f.write(str(world.n_cols))
    f.write("\n")
...
#            
    ...
    f.write("route Q-Learn:")
    f.write(str(route_q_learn))
    f.write("\n")
...
#    ##Data Saving the Matrices
#    for i in range(world.n_rows):
#        for j in range(world.n_cols):

#    np.save(f_sarsa,q_value_sarsa)
#    np.save(f_q_learn,q_value_q_learn)
    
    f.write("Q Value function SARSA: ")
    f.write(str(q_value_sarsa))
    f.write("\n \t \n")
    f.write("Q Value function SARSA: ")
    f.write(str(q_value_q_learn))
    f.write("\n \t \n")
    f.close
