...
class env():
    ...
    def simulator(self,cur_State,action):
        ...
        return np.array([th1_new, th1_Dot_new, th2_new, th2_Dot_new])
...
class basis_fn():
    def __init__(self,f_order):
        self.f_order = f_order
        x = np.arange(0,f_order+1)
        self.c = [p for p in itertools.product(x, repeat=4)]
        self.phi = np.zeros(len(self.c))
    
    def fourier(self,cur_state):
#        n_dim = len(cur_state)      
        scaled_state = np.array([cur_state[0]/(2*np.pi), cur_state[1]/(8*np.pi), \
                        cur_state[2]/(2*np.pi), cur_state[3]/(18*np.pi)])
        
        self.phi = np.cos(np.pi*np.dot(self.c,scaled_state))
...
#        c = [p for p in itertools.product(x, repeat=4)]
        ...
        alpha2 = alpha*np.ones(len(self.c))
...
class policy():
    def e_greedy(self, weights,phi,n_eps):
        ...
        if p<eps:
            action = np.array([rand.randint(-1,2,1)])
        else:
#            print "np.dot(weights,phi) =", np.dot(weights,phi)
...
#           new_Q = np.dot(weights[action+1,:],new_phi)
           ...
           weights[action+1,:] = weights[action+1,:] + delta*np.dot(alpha2,phi)
...
#==============================================================================
   ...
   ## Final Policy
    ...
    while(tip[1] < world.l1 and counter2<100):
        phi = basis.fourier(cur_state)
